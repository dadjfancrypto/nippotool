...
- **絶対に守ってください：入力テキストに存在しない人物名（例：田中さん、佐藤さんなど）は一切生成しないでください。人物名が入力に含まれていない場合は、人物名を一切使用せず、その部分を省略するか匿名化してください。**
...
【日付・年の扱い】
- 入力に日付（例：27日、3月15日）が出てきて年が言及されていない場合は、**常に現在の年**を使って補完すること。今日の日付は「{current_date}」なので、現在の年は {current_year} 年である。ユーザーが年を言わなくても、この年で表記すること。
